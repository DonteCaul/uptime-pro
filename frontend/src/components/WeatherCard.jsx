...
function kphToKt(kph) { return kph != null ? kph * 0.54 : null; }
function kphToMph(kph) { return kph != null ? kph * 0.621 : null; }
function cToF(c) { return c != null ? (c * 9) / 5 + 32 : null; }

// Wind direction arrow — rotated ↑ to point where wind blows TO
function WindArrow({ deg, color, size = 11 }) {
  ...
  return (
    ...
    >
      <line x1="5" y1="9" x2="5" y2="1.5" stroke={color} strokeWidth="2" strokeLinecap="round" />
      <polyline points="2.5,4.5 5,1.5 7.5,4.5" stroke={color} strokeWidth="1.8" fill="none" strokeLinejoin="round" />
    </svg>
  );
}

// Single wind level row with bar
function WindRow({ label, altFt, kph, dirDeg, dir, gustKph, isTopLevel }) {
    ...
    <div className="flex flex-col gap-0.5">
      ...
      <div className="flex items-center justify-between text-[9px]">
        ...
        <div className="flex items-center gap-1 shrink-0">
          ...
          {gustStr && (
            <span className="text-muted-foreground">({gustStr})</span>
          )}
          <span
            className="text-[8px] font-bold ml-1 px-1 rounded"
            style={{ color, background: THRESHOLDS.find(t => (kt ?? 0) < t.max)?.bg ?? '#ef444420' }}
          >
      ...
      {/* Bar */}
      <div className="relative h-3 rounded overflow-hidden" style={{ background: '#ffffff0a' }}>
        <div
        ...
        />
        {/* Scale tick marks at 0, 20, 40, 60 kt */}
        {[0, 20, 40, 60].map(tick => (
          <div
            key={tick}
...
function overallCondition(weather) {
  const winds = [
  ...
  // Surface winds are most critical for landing safety
  if (surfaceKt >= 25 || gustKt >= 30 || maxKt >= 50) return { label: 'GROUNDED',  color: '#ef4444' };
  if (surfaceKt >= 18 || gustKt >= 22 || maxKt >= 40) return { label: 'MARGINAL',  color: '#f97316' };
  if (surfaceKt >= 12 || gustKt >= 16 || maxKt >= 30) return { label: 'CAUTION',   color: '#eab308' };
  if (surfaceKt >= 6  || maxKt >= 15)                  return { label: 'GOOD',      color: '#84cc16' };
  ...
  if (loading) {
    return (
      <section className="border border-border rounded-lg p-4">
        ...
        <p className="text-xs text-muted-foreground animate-pulse">Fetching weather data…</p>
      </section>
    );
  }
    ...
    <section className="border border-border rounded-lg overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-2 border-b border-border">
        ...
        <span
          ...
          style={{ color: condition.color, background: condition.color + '20', border: `1px solid ${condition.color}40` }}
        >
          {condition.label}
        </span>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 px-4 pt-3 pb-2 text-sm">
        ...
        {weather.cloud_pct != null && (
          <span><CloudIcon pct={weather.cloud_pct} /> {Math.round(weather.cloud_pct)}%</span>
        )}
        {weather.precip_mm != null && weather.precip_mm > 0 && (
          <span className="text-sky-400">🌧 {weather.precip_mm.toFixed(1)} mm</span>
        )}
      </div>
